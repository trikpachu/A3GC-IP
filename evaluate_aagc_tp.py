...
def evaluate_pose(dataset, num_past_frame=20, num_future_frame=5, model_path=''):
    ...
    net1 = PoseNet(input_size=12, rotsize=3, adjacency=nira, n_hidden=args.hu1, device=device).to(device)
    net2 = PoseNet(input_size=15, rotsize=3, adjacency=nira, n_hidden=args.hu2, device=device).to(device)
    net3 = PoseNet(input_size=15, rotsize=9, adjacency=nira, n_hidden=args.hu3, device=device).to(device)

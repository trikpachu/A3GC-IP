...
from datasets import GraphDataset_tp
from tqdm import tqdm
from net_aagc import PoseNet_GGRU, pose_loss
import pickle
import glob 
import re 
...
parser.add_argument('--patience', type=int, default=3, help='gpu to use')
...
def valid_one_epoch(val_loader, model, criterion, refine=False, model_number=1):
    ...
    for i, (imu, leaf_pos_input, full_pos_input, leaf_pos, full_pos, smpl) in bar:
        if model_number == 1:
            ...
            inputs = torch.cat((imu, leaf_pos.view(leaf_pos.shape[0], leaf_pos.shape[1], 15, 3)), dim=-1)
            target = full_pos
        elif model_number == 3:
            inputs = torch.cat((imu, full_pos.view(full_pos.shape[0], full_pos.shape[1], 15, 3)), dim=-1)
            target = smpl
...
def main():
    ...
    model1 = PoseNet_GGRU(input_size=12, rotsize=3, adjacency=nira, n_hidden=args.hu1, device=device).to(device)
    model2 = PoseNet_GGRU(input_size=15, rotsize=3, adjacency=nira, n_hidden=args.hu2, device=device).to(device)
    model3 = PoseNet_GGRU(input_size=15, rotsize=9, adjacency=nira, n_hidden=args.hu3, device=device).to(device)
    ...
    if args.resume:
        ...
        for i in model_list:
            ...
            if 'finetuning' in i:
                fine_dup = 1
        duplicity = pre_dup + fine_dup
        if duplicity == 2:
            model_list = [i for i in model_list if 'pretrain' in i]
        elif duplicity == 0:
            raise ValueError('Found neither savefiles with pretrain in their name, nor with finetuning')
    ...
    train_dataset = GraphDataset_tp(os.path.join(amass_path if not args.finetuning else dip_path, "train_tp_global_chunked_pos.pt"), rotsize=9, norm=args.norm, sym=args.cda)
    train_loader = torch.utils.data.DataLoader(train_dataset,
        batch_size=args.batch_size, shuffle=True,
        num_workers=args.workers, pin_memory=True)
    if args.fse:
        val_dataset = GraphDataset_tp(os.path.join(amass_path if not args.finetuning else dip_path, "valid_tp_global_pos.pt"), rotsize=9, norm=args.norm, sym=args.cda)
        val_loader = torch.utils.data.DataLoader(val_dataset,
            batch_size=1, shuffle=False,
            num_workers=args.workers, pin_memory=True)
    else:
        val_dataset = GraphDataset_tp(os.path.join(amass_path if not args.finetuning else dip_path, "valid_tp_global_chunked_pos.pt"), rotsize=9, norm=args.norm, sym=args.cda)
        val_loader = torch.utils.data.DataLoader(val_dataset,
